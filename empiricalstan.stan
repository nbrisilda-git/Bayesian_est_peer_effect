...
data{
    ...
    #int YY1[n1,n1]; //distance of alcohol at time t1
    #int YY2[n1,n1]; //-//- at time t2
    #int DD1[n1,n1]; #drug covariate
    #int DD2[n1,n1];
    #int SS1[n1,n1]; #smoke covariate
    #int SS2[n1,n1];
    #int SP1[n1,n1]; #sport covariate
    #int SP2[n1,n1];
     }
parameters {
    real<lower=0> prec;
...
model{
  ...
  alpha[s] ~ normal(0, 100); //regression parameters
   ...
   for (i in 1:(n1-1)){
    ...
    #+alpha[4]*YY1[i,j]+alpha[5]*DD1[i,j]+alpha[6]*SS1[i,j]+alpha[7]*SP1[i,j];
    real mu3 = alpha[1] + alpha[2]*PP2[i,j] + alpha[3]*ZZ1[i,j];
    #+alpha[4]*YY2[i,j]+alpha[5]*DD2[i,j]+alpha[6]*SS2[i,j]+alpha[7]*SP2[i,j];
